...
def serve_produits():
    return send_from_directory('.', 'produits.html')
...
def get_produits():
    ...
    try:
        ...
        # Récupérer tous les produits actifs
        cursor.execute("""
        SELECT id, nom, description, prix, categorie, image_url, images_urls, 
        ...
        for produit in produits:
            if produit.get('images_urls') and isinstance(produit['images_urls'], str):
                try:
                    import json
                    produit['images_urls'] = json.loads(produit['images_urls'])
                except:
                    produit['images_urls'] = []
            
            if produit.get('taille_disponible') and isinstance(produit['taille_disponible'], str):
                try:
                    import json
                    produit['taille_disponible'] = json.loads(produit['taille_disponible'])
                except:
                    produit['taille_disponible'] = []
                    
            if produit.get('couleur_disponible') and isinstance(produit['couleur_disponible'], str):
                try:
                    import json
                    produit['couleur_disponible'] = json.loads(produit['couleur_disponible'])
                except:
                    produit['couleur_disponible'] = []
