...
def creer_commande():
    ...
    # Accept either a logged-in session user_id or a fallback user_id provided in the JSON payload
    user_id = session.get('user_id') or data.get('user_id')
    if not user_id:
        print('Création commande - aucun user_id en session et aucun user_id fourni dans le payload')
        return jsonify({"success": False, "message": "Non authentifié"}), 401
    
    # Validation des données requises
    required_fields = ['montant_total', 'adresse_livraison', 'telephone', 'panier']
    ...
    try:
        ...
        print(f"Création commande reçue pour user_id={user_id} payload_keys={list(data.keys())}")
        # Insérer la commande
        query = """
            INSERT INTO commandes 
            (user_id, numero_commande, montant_total, adresse_livraison, telephone, notes, statut)
...
def mettre_a_jour_commande(commande_id):
    ...
    try:
        ...
        # Vérifier les droits
        cursor.execute("SELECT user_id, statut FROM commandes WHERE id = %s", (commande_id,))
        ...
        if not user or (not user['is_admin'] and commande['user_id'] != session['user_id']):
