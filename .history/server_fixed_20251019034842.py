...
def api_login():
    ...
    try:
        ...
        if user:
            # Type hint: user est un dictionnaire
            user_dict: Dict[str, Any] = dict(user) if user else {}
            print(f"✅ Utilisateur trouvé: {user_dict.get('email', '')}")
            ...
            if user_id:
                ...
                if new_conn:
                    ...
                    token = f"token_{user_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                    new_cursor.execute("""
                        UPDATE users
                        SET session_active = 1, token_session = %s, derniere_connexion = NOW(), ip_connexion = %s
                        WHERE id = %s
                    """, (token, request.remote_addr, user_id))
                    new_conn.commit()
                    new_cursor.close()
                    new_conn.close()
...
@app.route('/api/commandes/me', methods=['POST', 'OPTIONS'])
...
def creer_commande():
    ...
    data = request.get_json()
    print(f"📝 Création de commande, données reçues: {data}")
    
    # Validation des données requises
    required_fields = ['user_id', 'montant_total', 'adresse_livraison', 'telephone']
    ...
    try:
        ...
        query = """
            ...
            (user_id, numero_commande, montant_total, adresse_livraison, telephone, notes, statut, date_commande)
            VALUES (%s, %s, %s, %s, %s, %s, 'en_attente', NOW())
        """
        cursor.execute(query, (
            data['user_id'],
            numero_commande,
        ...
        print(f"✅ Commande créée avec succès - ID: {commande_id}, Numéro: {numero_commande}")
...
def get_commande_detail(commande_id):
    ...
    try:
        ...
        # Récupérer les détails de la commande avec infos utilisateur
        query = """
...
def mettre_a_jour_commande(commande_id):
    ...
    try:
        ...
        if 'adresse_livraison' in data:
            update_fields.append("adresse_livraison = %s")
            update_values.append(data['adresse_livraison'])
