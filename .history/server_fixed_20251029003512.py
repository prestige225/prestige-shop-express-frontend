...
def get_produits():
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        query = "SELECT * FROM produits WHERE statut = 'actif' ORDER BY date_creation DESC"
        cursor.execute(query)
        produits = cursor.fetchall()
        
        return jsonify({"success": True, "data": produits})
...
def get_produit_detail(produit_id):
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        query = "SELECT * FROM produits WHERE id = %s AND statut = 'actif'"
        ...
        return jsonify({"success": True, "data": produit})
...
@app.route('/api/produits/categorie/<string:categorie>', methods=['GET', 'OPTIONS'])
def get_produits_par_categorie(categorie):
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        query = "SELECT * FROM produits WHERE categorie = %s AND statut = 'actif' ORDER BY date_creation DESC"
