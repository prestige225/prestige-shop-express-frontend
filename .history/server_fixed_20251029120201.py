...
# Cache pour colonnes existantes (évite requêtes répétées)
_COLUMN_CACHE = {}

def db_has_column(table_name, column_name):
    """Vérifier si une colonne existe dans la base (cache simple)."""
...
def api_upload():
    ...
    kind = request.form.get('kind', 'image')  # 'image' or 'video'
    ...
    if kind == 'video':
        if not allowed_file(filename, ALLOWED_VIDEO_EXT):
            return jsonify({'success': False, 'message': 'Type de vidéo non autorisé'}), 400
    ...
    path = os.path.join(save_dir, filename_on_disk)
    try:
        file.save(path)
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

    file_url = f"{url_prefix}/{filename_on_disk}"
...
def get_produits():
    ...
    try:
        ...
        for p in produits:
            ...
            # Si la colonne videos_urls existe, privilégier sa valeur séparée.
            try:
                # images_urls peut être soit une liste (ancienne utilisation) soit un objet {images:[], videos:[]}
                if 'images_urls' in p and p['images_urls']:
                    parsed = json.loads(p['images_urls']) if isinstance(p['images_urls'], str) else p['images_urls']
                else:
                    parsed = None

                # Si la colonne videos_urls est fournie par la requête SQL, l'utiliser
                if 'videos_urls' in p and p['videos_urls']:
                    try:
                        v_parsed = json.loads(p['videos_urls']) if isinstance(p['videos_urls'], str) else p['videos_urls']
                    except Exception:
                        v_parsed = []
                else:
                    v_parsed = None
                ...
                if v_parsed is not None:
                    # utiliser images depuis parsed (si présent) et videos depuis v_parsed
                    if parsed is None:
                        p['images_urls'] = []
                    else:
                        if isinstance(parsed, dict):
                            p['images_urls'] = parsed.get('images', [])
                        elif isinstance(parsed, list):
                            p['images_urls'] = parsed
                        else:
                            p['images_urls'] = []

                    p['videos_urls'] = v_parsed if isinstance(v_parsed, list) else []
                else:
                    # aucun videos_urls séparé: tenter d'extraire videos depuis images_urls si c'est un objet
                    if parsed is None:
                        p['images_urls'] = []
                        p['videos_urls'] = []
                    else:
                        if isinstance(parsed, dict):
                            p['images_urls'] = parsed.get('images', [])
                            p['videos_urls'] = parsed.get('videos', [])
                        elif isinstance(parsed, list):
                            p['images_urls'] = parsed
                            p['videos_urls'] = []
                        else:
                ...
                print(f"⚠️ Erreur décodage JSON pour images/videos du produit {p.get('id')}: {e}")
                p['images_urls'] = []
                p['videos_urls'] = []

            for field in ('taille_disponible', 'couleur_disponible'):
                if field in p and p[field]:
                    try:
                        p[field] = json.loads(p[field]) if isinstance(p[field], str) else p[field]
                    except Exception as e:
                        print(f"⚠️ Erreur décodage JSON pour {field} du produit {p.get('id')}: {e}")
                        p[field] = []
...
def get_produit(produit_id):
    ...
    try:
        ...
        # Traiter images_urls et videos_urls : supporter colonne videos_urls séparée ou ancien format combiné
        try:
            parsed = None
            if 'images_urls' in produit and produit['images_urls']:
                parsed = json.loads(produit['images_urls']) if isinstance(produit['images_urls'], str) else produit['images_urls']

            v_parsed = None
            if 'videos_urls' in produit and produit.get('videos_urls'):
                try:
                    v_parsed = json.loads(produit['videos_urls']) if isinstance(produit['videos_urls'], str) else produit['videos_urls']
                except Exception:
                    v_parsed = []

            if v_parsed is not None:
                # use videos column and parsed images (if any)
                if parsed is None:
                    produit['images_urls'] = []
                else:
                    if isinstance(parsed, dict):
                        produit['images_urls'] = parsed.get('images', [])
                    elif isinstance(parsed, list):
                        produit['images_urls'] = parsed
                    else:
                        produit['images_urls'] = []

                produit['videos_urls'] = v_parsed if isinstance(v_parsed, list) else []
            else:
                # fallback: extract videos from parsed if it's a dict
                if parsed is None:
                    produit['images_urls'] = []
                    produit['videos_urls'] = []
                else:
                    if isinstance(parsed, dict):
                        produit['images_urls'] = parsed.get('images', [])
                        produit['videos_urls'] = parsed.get('videos', [])
                    elif isinstance(parsed, list):
                        produit['images_urls'] = parsed
                        produit['videos_urls'] = []
                    else:
...
def create_produit():
    ...
    # Supporter images_urls (array) et videos_urls (array).
    images_list = data.get('images_urls', []) if data.get('images_urls') is not None else []
    videos_list = data.get('videos_urls', []) if data.get('videos_urls') is not None else ([] if data.get('video_url') is None else [data.get('video_url')])

    # Vérifier si la colonne videos_urls existe : si oui, stocker séparément; sinon garder l'ancien format combiné
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'message': 'Erreur DB'}), 500

    has_videos_col = db_has_column('produits', 'videos_urls')

    images_urls_json = json.dumps(images_list, ensure_ascii=False)
    videos_urls_json = json.dumps(videos_list, ensure_ascii=False)
    ...
    try:
        if has_videos_col:
            cursor.execute(
                """
                INSERT INTO produits (nom, description, prix, categorie, image_url, images_urls, videos_urls, taille_disponible, couleur_disponible, stock, statut, date_creation, date_mise_a_jour)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())
                """,
                (nom, description, prix, categorie, image_url, images_urls_json, videos_urls_json, taille_disponible, couleur_disponible, stock, statut)
            )
        else:
            # backward-compatible: stocker un objet combiné dans images_urls
            combined = json.dumps({'images': images_list, 'videos': videos_list}, ensure_ascii=False)
            cursor.execute(
                """
                ...
                """,
                (nom, description, prix, categorie, image_url, combined, taille_disponible, couleur_disponible, stock, statut)
            )
...
def update_produit(produit_id):
    ...
    # Construire update dynamique en gérant séparément images/videos
    champs = []
    valeurs = []

    # Ouvrir connexion et récupérer l'état actuel des images/videos pour fusion
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'message': 'Erreur DB'}), 500
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute('SELECT images_urls FROM produits WHERE id = %s', (produit_id,))
        ...
        # Parse existing images_urls column (peut être liste ou objet)
        existing_images = []
        existing_videos = []
        if existing.get('images_urls'):
            try:
                parsed = json.loads(existing['images_urls']) if isinstance(existing['images_urls'], str) else existing['images_urls']
                if isinstance(parsed, dict):
                    existing_images = parsed.get('images', []) or []
                    existing_videos = parsed.get('videos', []) or []
                elif isinstance(parsed, list):
        ...
        # Fermer ce curseur avant d'en ouvrir un autre pour la mise à jour
        cursor.close()

        # Construire champs à mettre à jour à partir des données reçues
        for key, val in data.items():
            if key in ('taille_disponible', 'couleur_disponible'):
                ...
                champs.append(f"{key} = %s")
                valeurs.append(val)
            # images_urls et videos_urls seront gérés après la boucle pour merge

        # Gérer fusion des images/videos : si l'une ou l'autre est fournie, construire les nouvelles listes
        if 'images_urls' in data or 'videos_urls' in data:
            new_images = data.get('images_urls', existing_images) or []
            new_videos = data.get('videos_urls', existing_videos) or []
            # Si la colonne videos_urls existe dans la table, stocker séparément
            if db_has_column('produits', 'videos_urls'):
                champs.append('images_urls = %s')
                valeurs.append(json.dumps(new_images, ensure_ascii=False))
                champs.append('videos_urls = %s')
                valeurs.append(json.dumps(new_videos, ensure_ascii=False))
            else:
                # backward-compatible: stocker un objet combiné dans images_urls
                combined = json.dumps({'images': new_images, 'videos': new_videos}, ensure_ascii=False)
                champs.append('images_urls = %s')
                valeurs.append(combined)
        ...
        update_query = f"UPDATE produits SET {', '.join(champs)} WHERE id = %s"
        valeurs.append(produit_id)

        # Exécuter la mise à jour
        cursor = conn.cursor()
        cursor.execute(update_query, tuple(valeurs))
        conn.commit()
        cursor.close()
        conn.close()
