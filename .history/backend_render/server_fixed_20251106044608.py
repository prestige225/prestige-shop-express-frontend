...
def get_commandes():
    ...
    try:
        ...
        for i, commande in enumerate(commandes[:3]):  # Afficher les 3 premières commandes
            print(f"Commande {i+1}: ID={commande.get('id')}, User_ID={commande.get('user_id')}, "
                  f"Nom={commande.get('nom')}, Numéro={commande.get('user_numero')}")
...
def get_produits():
    ...
    try:
        ...
        for p in produits:
            ...
            for field in ('images_urls', 'videos_urls', 'taille_disponible', 'couleur_disponible'):
...
def create_produit():
    ...
    videos_urls = json.dumps(data.get('videos_urls', []), ensure_ascii=False)
    ...
    try:
        cursor.execute(
            ...
            """,
            (nom, description, prix, categorie, image_url, images_urls, videos_urls, taille_disponible, couleur_disponible, stock, statut)
        )
        conn.commit()
...
def update_produit(produit_id):
    ...
    champs = []
    valeurs = []
    mapping_json = ('images_urls', 'videos_urls', 'taille_disponible', 'couleur_disponible')
